...
// Day 4 - simple one liner
const day4 = `  "1-4": "# Slight Revision and Few Logical Problems",`;

// Day 5 - full content (I'll add as raw string)
const day5Content = `# Day 5 — Making Numbers Actually Work
...
- Ask for target runs and overs remaining
- Calculate: runs still needed, current run rate, required run rate to win
...
- Add an ideal weight range — for a given height, the healthy BMI range is 18.5-24.9:
...
let escaped = day5Content
  .replace(/\\/g, '\\\\')
  .replace(/"/g, '\\"')
  .replace(/\n/g, '\\n');

// Handle triple backticks - use 3 single quotes in the output
escaped = escaped.replace(/'''python/g, "\\'\\'\\'python");
escaped = escaped.replace(/'''/g, "\\'\\'\\'");

const day5 = `  "1-5": "${escaped}",`;

// Find the end of the content object - find "};  " then export
const endIdx = content.indexOf('};  \nexport');
if (endIdx === -1) {
  console.log('ERROR: Could not find end, trying alternate...');
  // Try different pattern
  const altIdx = content.indexOf('};');
  if (altIdx > 0) {
    const before = content.slice(0, altIdx + 2);
    const after = content.slice(altIdx + 2);
    const newContent = before + ',\n' + day4 + ',\n' + day5 + '\n' + after;
    fs.writeFileSync(pagePath, newContent);
    console.log('SUCCESS (alt): Added Day 4 and Day 5');
  } else {
    console.log('ERROR: Still could not find end');
  }
} else {
  const before = content.slice(0, endIdx + 2);
  const after = content.slice(endIdx + 2);
  const newContent = before + ',\n' + day4 + ',\n' + day5 + '\n' + after;
  fs.writeFileSync(pagePath, newContent);
  console.log('SUCCESS: Added Day 4 and Day 5');
}