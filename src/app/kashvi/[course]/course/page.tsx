...
`,
  ...
  "2-5":
    '# Week 2, Day 5 — Lists, Tuples & Sets\n\n### Anchor Project: Classroom Manager\n\n---\n\n## 🔁 Where We Left Off\n\nDay 2 introduced lists — create, access, change, add, remove.\n\nToday we go deeper on lists, then introduce two new data structures that solve problems lists can\'t handle cleanly.\n\nWe\'ll build a **Classroom Manager** step by step. Each concept enters the moment the current approach hits a wall.\n\n---\n\n## 🏗️ The Project\n\nYou are building a system to manage a classroom.\n\nIt needs to:\n\n- Track and analyse student scores\n- Store student records that should never be accidentally changed\n- Manage a clean list of unique enrolled subjects\n\nLet\'s build it stage by stage.\n\n---\n\n## 📊 Stage 1: Score Tracker\n\n### The Problem\n\nYou have ten student scores. You need to answer:\n\n- What is the total and average?\n- Who scored highest and lowest?\n- What does the ranked order look like?\n\n```python\nscores = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\n```\n\n---\n\n### `sum()`, `min()`, `max()`\n\nThese built-in functions work directly on any number list:\n\n```python\nscores = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\n\nprint(sum(scores))    # 757  ← total of all values\nprint(min(scores))    # 55   ← smallest value\nprint(max(scores))    # 91   ← largest value\n```\n\nAverage — combine `sum()` and `len()`:\n\n```python\naverage = round(sum(scores) / len(scores), 2)\nprint(average)    # 75.7\n```\n\n---\n\n### The Copying Problem\n\nYou want a sorted ranking list — but you don\'t want to destroy the original order.\n\n```python\nscores        = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\nranked_scores = scores          # ← looks fine\n\nranked_scores.sort()\nprint(ranked_scores)    # [55, 61, 65, 72, 74, 78, 83, 88, 90, 91]\nprint(scores)           # [55, 61, 65, 72, 74, 78, 83, 88, 90, 91]  😱\n```\n\nBoth changed. `ranked_scores = scores` doesn\'t copy — it creates a second name pointing to the **same list** in memory.\n\n**The fix:**\n\n```python\nranked_scores = scores.copy()    # Method 1 — explicit copy\nranked_scores = scores[:]        # Method 2 — slice copy\n```\n\nNow they are independent:\n\n```python\nscores        = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\nranked_scores = scores.copy()\n\nranked_scores.sort(reverse=True)\nprint(ranked_scores)    # [91, 90, 88, 83, 78, 74, 72, 65, 61, 55]\nprint(scores)           # [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]  ✅ unchanged\n```\n\n---\n\n### Classroom Manager — Stage 1\n\n```python\nscores        = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\nranked_scores = scores.copy()\nranked_scores.sort(reverse=True)\n\nprint("=" * 38)\nprint("   SCORE REPORT")\nprint("=" * 38)\nprint(f"Students  : {len(scores)}")\nprint(f"Average   : {round(sum(scores) / len(scores), 2)}")\nprint(f"Highest   : {max(scores)}")\nprint(f"Lowest    : {min(scores)}")\nprint(f"Ranked    : {ranked_scores}")\nprint("=" * 38)\n```\n\n```\n========================================\n   SCORE REPORT\n========================================\nStudents  : 10\nAverage   : 75.7\nHighest   : 91\nLowest    : 55\nRanked    : [91, 90, 88, 83, 78, 74, 72, 65, 61, 55]\n========================================\n```\n\n---\n\n### ✏️ Practice 1\n\nA new student joined with a score of `87`. Also, the score `55` was a data entry error — remove it.\n\nStart from:\n\n```python\nscores = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\n```\n\n- Add `87` to the end\n- Remove `55` by value\n- Create a safe copy and sort it descending\n- Print: total students, average (2dp), highest, lowest, ranked list\n- Print whether any score is below `60` using `min()` and comparison — `True` or `False`\n\n<details>\n<summary>Show answer</summary>\n\n```python\nscores = [72, 88, 65, 91, 55, 78, 83, 90, 61, 74]\n\nscores.append(87)\nscores.remove(55)\n\nranked = scores.copy()\nranked.sort(reverse=True)\n\nprint(f"Students  : {len(scores)}")\nprint(f"Average   : {round(sum(scores) / len(scores), 2)}")\nprint(f"Highest   : {max(scores)}")\nprint(f"Lowest    : {min(scores)}")\nprint(f"Ranked    : {ranked}")\nprint(f"Below 60  : {min(scores) < 60}")\n```\n\n</details>\n\n---\n\n## 📌 Stage 2: Student Records — Enter Tuples\n\n### The New Problem\n\nYou need to store each student\'s **ID, name, and score** together.\n\nYou try a list:\n\n```python\nstudent = [101, "Prassist Thapa", 91]\nstudent[0] = 999    # 😬 accidentally overwrites the ID\n```\n\nStudent IDs and names are **fixed facts** — they should never change once created. You need a structure that looks like a list but refuses to be modified.\n\nThat\'s a **tuple**.\n\n---\n\n### What is a Tuple?\n\nA tuple is an **ordered, immutable** collection. Created with parentheses `()` instead of square brackets:\n\n```python\nstudent = (101, "Prassist Thapa", 91)\n```\n\nAccess works exactly like a list:\n\n```python\nprint(student[0])     # 101\nprint(student[1])     # \'Prassist Thapa\'\nprint(student[-1])    # 91\n```\n\nBut modification crashes intentionally:\n\n```python\nstudent[0] = 999    # 💥 TypeError: \'tuple\' object does not support item assignment\n```\n\n> This crash is a feature, not a bug. It protects your data.\n\n---\n\n### Creating Tuples\n\n```python\nstudent     = (101, "Prassist Thapa", 91)\ncoordinates = (27.7172, 85.3240)        # latitude, longitude\ncolour      = (255, 87, 51)             # RGB value\nsingle      = (42,)                     # ← trailing comma required for single item\nempty       = ()\n```\n\n> `(42)` is just the number `42` in parentheses. `(42,)` is a tuple containing `42`. The comma makes the difference.\n\n---\n\n### Tuple Operations\n\n```python\nstudent = (101, "Prassist Thapa", 17, "Kathmandu")\n\nprint(len(student))              # 4\nprint(student[1:3])              # (\'Prassist Thapa\', 17)\nprint("Prassist Thapa" in student)  # True\nprint(student.count(17))         # 1\nprint(student.index(17))         # 2\n```\n\nWhat tuples **do not** support:\n\n```python\nstudent.append("extra")    # 💥 AttributeError\nstudent[0] = 200           # 💥 TypeError\n```\n\n---\n\n### Unpacking — Python\'s Cleanest Pattern\n\nAssign each value to its own variable in one line:\n\n```python\nstudent = (101, "Prassist Thapa", 91)\n\nsid, name, score = student\n\nprint(sid)      # 101\nprint(name)     # \'Prassist Thapa\'\nprint(score)    # 91\n```\n\nWorks with any sequence. Common pattern with coordinates:\n\n```python\nlat, lng = (27.7172, 85.3240)\nprint(f"Lat: {lat} | Lng: {lng}")\n```\n\n---\n\n### Classroom Manager — Stage 2\n\n```python\nstudents = [\n    (101, "Prassist Thapa",    91),\n    (102, "Arjun Karki",       78),\n    (103, "Kabir Shrestha",    88),\n    (104, "Zara Malla",        65),\n    (105, "Riya Gurung",       83),\n]\n\nprint("=" * 40)\nprint("   STUDENT RECORDS")\nprint("=" * 40)\nprint(f"{\'ID\':<6} {\'Name\':<20} {\'Score\'}")\nprint("-" * 40)\n\nfor student in students:\n    sid, name, score = student\n    print(f"{sid:<6} {name:<20} {score}")\n\nprint("=" * 40)\n```\n\n```\n========================================\n   STUDENT RECORDS\n========================================\nID     Name                 Score\n----------------------------------------\n101    Prassist Thapa       91\n102    Arjun Karki          78\n103    Kabir Shrestha       88\n104    Zara Malla           65\n105    Riya Gurung          83\n========================================\n```\n\n> `f"{ \'ID\':<6}"` is f-string alignment — `<6` means left-align within 6 characters. This makes columns line up neatly.\n\n---\n\n### ✏️ Practice 2\n\nAdd two more students to the list:\n\n- `(106, "Dev Rana", 72)`\n- `(107, "Sneha Joshi", 95)`\n\nThen:\n\n- Print total student count\n- Unpack the last student\'s tuple and print their name and score separately\n- Print the name of the student at index 2 using chained indexing: `students[2][1]`\n- Collect all scores into a new list using unpacking in a loop — then print the average\n\n```python\nstudents = [\n    (101, "Prassist Thapa",    91),\n    (102, "Arjun Karki",       78),\n    (103, "Kabir Shrestha",    88),\n    (104, "Zara Malla",        65),\n    (105, "Riya Gurung",       83),\n]\n# add two students\n# total count\n# unpack last student\n# name at index 2\n# collect all scores and find average\n```\n\n<details>\n<summary>Show answer</summary>\n\n```python\nstudents = [\n    (101, "Prassist Thapa",    91),\n    (102, "Arjun Karki",       78),\n    (103, "Kabir Shrestha",    88),\n    (104, "Zara Malla",        65),\n    (105, "Riya Gurung",       83),\n]\n\nstudents.append((106, "Dev Rana",       72))\nstudents.append((107, "Sneha Joshi",    95))\n\nprint("Total:", len(students))\n\nsid, name, score = students[-1]\nprint(f"Last student: {name} — {score}")\n\nprint("Index 2 name:", students[2][1])\n\nscores = []\nfor student in students:\n    _, _, s = student\n    scores.append(s)\n\nprint(f"Average: {round(sum(scores) / len(scores), 2)}")\n```\n\n</details>\n\n---\n\n## 🔵 Stage 3: Unique Subjects — Enter Sets\n\n### The New Problem\n\nStudents enrol in subjects. After collecting enrollments, the list has duplicates:\n\n```python\n enrolled = [\n     "Maths", "Science", "English", "Maths",\n     "Computer", "English", "Science", "Maths",\n     "Nepali", "Computer"\n ]\n```\n\nYou need the unique subjects only. A set solves this instantly.\n\n---\n\n### What is a Set?\n\nA set is an **unordered collection of unique values**. Duplicates are removed automatically.\n\nCreated with curly braces `{}` or `set()`:\n\n```python\nsubjects = {"Maths", "Science", "English", "Computer"}\n\n# Convert a list — duplicates removed instantly\nunique = set(enrolled)\nprint(unique)    # {\'Maths\', \'Science\', \'English\', \'Computer\', \'Nepali\'}\n```\n\n---\n\n### Key Properties\n\n| Property  | Meaning                          |\n| --------- | -------------------------------- |\n| Unordered | No fixed position — no indexing  |\n| Unique    | Duplicates removed automatically |\n| Mutable   | Can add and remove items         |\n\n```python\nsubjects[0]    # 💥 TypeError — sets have no index\n```\n\n> Sets trade ordering for a guarantee: **every value appears exactly once.**\n\n---\n\n### Creating Sets\n\n```python\nsubjects   = {"Maths", "Science", "English"}\nfrom_list  = set(["a", "b", "a", "c", "b"])    # → {\'a\', \'b\', \'c\'}\nempty_set  = set()    # ← NOT {} — that creates a dictionary!\n```\n\n---\n\n### Adding and Removing\n\n```python\nsubjects = {"Maths", "Science", "English"}\n\nsubjects.add("Computer")       # adds new item\nsubjects.add("Maths")          # already exists — no error, no duplicate\nsubjects.remove("Science")     # removes — crashes if not found\nsubjects.discard("Physics")    # removes if found — silent if not found\n```\n\n> Use `.discard()` when unsure if the item exists. Use `.remove()` when it must be there.\n\n---\n\n### Set Operations\n\n```python\nclass_A = {"Maths", "Science", "English", "Computer"}\nclass_B = {"Maths", "Nepali", "English", "Art"}\n\nprint(class_A | class_B)    # Union — all subjects from both\nprint(class_A & class_B)    # Intersection — subjects in both\nprint(class_A - class_B)    # Difference — in A but not B\nprint(class_A ^ class_B)    # Symmetric diff — in one but not both\n```\n\n```\n{\'Maths\', \'Science\', \'English\', \'Computer\', \'Nepali\', \'Art\'}\n{\'Maths\', \'English\'}\n{\'Science\', \'Computer\'}\n{\'Science\', \'Computer\', \'Nepali\', \'Art\'}\n```\n\n---\n\n### Classroom Manager — Stage 3\n\n```python\nprassist_subs = {"Maths", "Science", "English", "Computer"}\narjun_subs   = {"Maths", "Nepali", "English", "Art"}\nkabir_subs   = {"Science", "Computer", "Art", "PE"}\n\nall_subs    = prassist_subs | arjun_subs | kabir_subs\ncommon_subs = prassist_subs & arjun_subs & kabir_subs\nprassist_only  = prassist_subs - arjun_subs - kabir_subs\n\nprint("=" * 40)\nprint("   SUBJECT ANALYSIS")\nprint("=" * 40)\nprint(f"All subjects  : {sorted(all_subs)}")\nprint(f"Common to all : {sorted(common_subs)}")\nprint(f"Prassist only    : {sorted(prassist_only)}")\nprint(f"Total unique  : {len(all_subs)}")\nprint("=" * 40)\n```\n\n```\n========================================\n   SUBJECT ANALYSIS\n========================================\nAll subjects  : [\'Art\', \'Computer\', \'English\', \'Maths\', \'Nepali\', \'PE\', \'Science\']\nCommon to all : []\nPrassist only    : [\'Maths\']\nTotal unique  : 7\n========================================\n```\n\n> `sorted()` on a set gives a consistently ordered list — sets print in random order otherwise.\n\n---\n\n### ✏️ Practice 3\n\nYou have two-day attendance data:\n\n```python\nday1 = {"Prassist", "Arjun", "Kabir", "Zara", "Riya"}\nday2 = {"Arjun", "Kabir", "Dev", "Sneha", "Prassist"}\n```\n\nFind and print:\n\n- Students present **both days**\n- Students present on **at least one** day\n- Present on Day 1 but **absent** on Day 2\n- Present on Day 2 but **absent** on Day 1\n- Total unique students across both days\n- Whether `"Zara"` attended Day 2 — `True` or `False`\n\n<details>\n<summary>Show answer</summary>\n\n```python\nday1 = {"Prassist", "Arjun", "Kabir", "Zara", "Riya"}\nday2 = {"Arjun", "Kabir", "Dev", "Sneha", "Prassist"}\n\nprint("Both days    :", sorted(day1 & day2))\nprint("Either day   :", sorted(day1 | day2))\nprint("Day1 not Day2:", sorted(day1 - day2))\nprint("Day2 not Day1:", sorted(day2 - day1))\nprint("Total unique :", len(day1 | day2))\nprint("Zara on Day2 :", "Zara" in day2)\n```\n\n</details>\n\n---\n\n## 📊 The Three Structures — Side by Side\n\n|            | List                 | Tuple         | Set           |\n| ---------- | -------------------- | ------------- | ------------- |\n| Syntax     | `[1, 2, 3]`          | `(1, 2, 3)`   | `{1, 2, 3}`   |\n| Ordered    | ✅                   | ✅            | ❌            |\n| Indexed    | ✅                   | ✅            | ❌            |\n| Mutable    | ✅                   | ❌            | ✅            |\n| Duplicates | ✅                   | ✅            | ❌            |\n| Best for   | Changing collections | Fixed records | Unique values |\n\n---\n\n## 🎯 Challenge — Classroom Manager (Combined)\n\nBuild the complete classroom manager combining all three structures.\n\n**Setup:**\n\n```python\nstudents = [\n    (101, "Prassist Thapa",    91, {"Maths", "Science", "English"},\n    (102, "Arjun Karki",       78, {"Maths", "Nepali", "PE"}),\n    (103, "Kabir Shrestha",    88, {"Science", "English", "Art"}),\n    (104, "Zara Malla",        65, {"Maths", "English", "Computer"}),\n    (105, "Riya Gurung",       83, {"Science", "Computer", "PE"}),\n]\n```\n\nEach tuple: `(id, name, score, subjects_set)`\n\n**Requirements:**\n\n1. Print a formatted student table — ID, name, score (use f-string alignment)\n2. Collect all scores into a list using unpacking in the loop\n3. Print: total students, average (2dp), highest, lowest, ranked descending\n4. Find all unique subjects across all students — union of all subject sets\n5. Find subjects studied by **every** student — intersection of all sets\n6. Print the name of the top scoring student — unpack their tuple\n7. Print whether `"Computer"` appears in the full subject union — `True` or `False`\n\n**Expected output:**\n\n```\n========================================\n   CLASSROOM MANAGER\n========================================\nID     Name                 Score\n----------------------------------------\n101    Prassist Thapa       91\n102    Arjun Karki          78\n103    Kabir Shrestha       88\n104    Zara Malla           65\n105    Riya Gurung          83\n========================================\nTotal     : 5\nAverage   : 81.0\nHighest   : 91\nLowest    : 65\nRanked    : [91, 88, 83, 78, 65]\n----------------------------------------\nAll subjects  : [\'Art\', \'Computer\', \'English\', \'Maths\', \'Nepali\', \'PE\', \'Science\']\nCommon to all : [\'English\']\nTop student   : Prassist Thapa\nHas Computer  : True\n========================================\n```\n\n> Hints:\n>\n> - Collect scores: start with `scores = []`, loop through students, unpack with `_, _, s, _ = student`, then `scores.append(s)`\n> - Union: start `all_subs = set()`, then `all_subs = all_subs | student[3]` each iteration\n> - Intersection: start `common = students[0][3].copy()`, then `common = common & student[3]` for each remaining student\n> - Top student: find `max(scores)`, then loop to find which student has that score and unpack their name\n\n---\n\n## ✅ What You Learned Today\n\n**Lists (deeper):**\n\n- `sum()`, `min()`, `max()` — analyse number lists in one call\n- `.copy()` and `[:]` — create independent copies that don\'t affect the original\n\n**Tuples:**\n\n- Created with `()` — immutable, ordered, indexed\n- Crash on modification — that\'s the point\n- Unpacking: `a, b, c = (1, 2, 3)` — clean, readable, widely used\n- Best for fixed records: student IDs, coordinates, RGB values\n\n**Sets:**\n\n- Created with `{}` or `set()` — mutable, unordered, unique values only\n- `.add()`, `.remove()`, `.discard()`\n- Operations: `|` union, `&` intersection, `-` difference, `^` symmetric difference\n- Fast membership check with `in`\n- Wrap in `sorted()` for consistent print order\n\n---\n\n## 👀 Coming Up — Day 6\n\n**??**\n'
};
